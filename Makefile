...
clean:
	...
	docker image rm --force $(DOCKER_IMAGE_PREFIX)/taiga-back-azure:$(TAIGA_BACK_TAG)
	docker image rm --force $(DOCKER_IMAGE_PREFIX)/taiga-front-azure:$(TAIGA_FRONT_TAG)
...
build-front:
	docker build docker/front $(DOCKER_BUILD_OPTS) --tag $(DOCKER_IMAGE_PREFIX)/taiga-front-azure:$(TAIGA_FRONT_TAG)

build-back:
	docker build docker/back $(DOCKER_BUILD_OPTS) --tag $(DOCKER_IMAGE_PREFIX)/taiga-back-azure:$(TAIGA_BACK_TAG)
	
publish:
	docker push $(DOCKER_IMAGE_PREFIX)/taiga-back-azure:$(TAIGA_BACK_TAG)
	docker push $(DOCKER_IMAGE_PREFIX)/taiga-front-azure:$(TAIGA_FRONT_TAG)
